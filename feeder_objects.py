...
def trips(counter, from, node, label, hsPhase, lsPhase, lineConfig):
    node_OL = overhead_lines_objects(f"OL_{node}_{label}_xfmr_{node}_{label}_{counter}", hsPhase, f"meter_{node}_A",f"xfmr_meter_{node}_{label}_{counter}", lineConfig)
    print(node_OL)
...
#    print(trip_line)
    ...
    for k in range(1,9):
        ...
        trip_lines = triplex_line(f"trip_line_{node}_{label}_{trip_line_counter}", from, meterName, f"AS")
...
for (i, (node, label, hsPhase, lsPhase, lineConfig)) in enumerate(nodes):
    for j in range(1, 6):
        trips(i, f"trip_node_{node}_{label}_{j}", node, label, hsPhase, lsPhase, lineConfig)